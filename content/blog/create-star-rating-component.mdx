...
I have added some defaults, but you can of course change then to your liking. 


## Step 4: Implementing Interactive Handlers

Let's add the handlers for click and hover interactions. These will be needed to create a nice hover offect, in a way creating a sort of rating "preview", before the new rating is clicked.
...
We assign a const `ratingToUse` and then we use such variable to determine which color / fill a "star" should have. 


## Step 6: Generating the Stars

Then, all we need to do is do display the star icons as a dynamic array whose lenght is determined by the `maxStars` prop.
